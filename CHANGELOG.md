...
### Added
- **[What Is the Fairness-Accuracy Trade-off?](explainers/fairness-accuracy-tradeoff.md)** (closes #345, by [@propcgamer20-png](https://github.com/propcgamer20-png), [#482](https://github.com/yakew7/Fair-Code/pull/482)) - why closing a fairness gap often costs predictive accuracy and the two aren't always improved together, with a real S0-S4 demographic-parity-vs-accuracy table from the COMPAS audit, cross-verified against `paper/results-frozen/`.
- **[What Is Treatment Equality?](explainers/treatment-equality.md)** (closes #344, by [@propcgamer20-png](https://github.com/propcgamer20-png), [#481](https://github.com/yakew7/Fair-Code/pull/481)) - why two groups with matching TPR/FPR gaps can still have opposite-skewed error types underneath, with a real 0.13-vs-31.0 FN:FP ratio split computed from the COMPAS baseline model, cross-verified against a fresh run of the same model/split.
- **[What Is a Multiple-Comparisons Correction (Bonferroni, Holm, and False Discovery Rate)?](explainers/multiple-comparisons.md)** (closes #346) - why running dozens of p-values per audit (90 for COMPAS alone, 1,320 across the full frozen benchmark) inflates the odds of a false "significant" reading, applying Bonferroni, Holm, and Benjamini-Hochberg FDR to a real 90-p-value COMPAS/race slice from `paper/results-frozen/results_fairness.csv`, cross-verified against `statsmodels`. Notes as a limitation that none of these corrections are actually applied in `faircode/significance.py`/`faircode/benchmark.py` today.
...
### Fixed
- **`TargetSpec.compute()` silently turned a missing/NaN target label into a plain negative-class row** (closes #488, by [@propcgamer20-png](https://github.com/propcgamer20-png), [#510](https://github.com/yakew7/Fair-Code/pull/510)) - `NaN == value`, `NaN in [...]`, and `NaN > median` all evaluate to `False` in pandas, so `equals`/`isin`/`above_median` silently scored an unlabeled row as a confirmed negative instead of excluding it, while `binary` raised `IntCastingNaNError` on the same input - inconsistent with each other and with `ProtectedAttribute.disadvantaged_mask()`'s existing `known_mask` exclusion. `compute()` now returns `(y, known_mask)`, mirrored the same way for protected attributes; `faircode/benchmark.py`'s `_load_dataset` ANDs both together before filtering.
- **Non-numeric age sentinels (`"unknown"`, `"prefer not to say"`) were silently folded into `missing_pct` instead of kept as a categorical group** (closes #487, by [@propcgamer20-png](https://github.com/propcgamer20-png), [#509](https://github.com/yakew7/Fair-Code/pull/509)) - contradicted `faircode/SPEC.md` section 2's own documented "anything else: treat as categorical" rule for free text with no embedded number. A genuinely missing cell and an out-of-range numeric sentinel (e.g. `-1`) both still correctly count as missing; only true free text now gets its own group, in both engines.
- **`scripts/check_generated_files_current.py` never rebuilt from source - it only diffed the working tree against the last git commit** (closes #502, by [@propcgamer20-png](https://github.com/propcgamer20-png), [#508](https://github.com/yakew7/Fair-Code/pull/508)) - editing an explainer's `.md` without running `make build-explainers` afterward still reported "up to date" as long as nothing was committed yet. The script now builds a real fresh regeneration via a `git worktree` (with every currently-tracked file's working-tree content copied on top) and diffs against that instead of `HEAD`.
- **`faircode/SPEC.md` still documented `proxy_hints`'s `min_share`/`min_group_size` params, which #471 already removed from the code** (closes #499, by [@propcgamer20-png](https://github.com/propcgamer20-png), [#507](https://github.com/yakew7/Fair-Code/pull/507)) - a doc-only leftover from that fix; SPEC.md now says the tool takes no threshold parameters at all.
- **`make check` gave false confidence - it never ran any of `audits.yml`'s required checks** (closes #492, by [@propcgamer20-png](https://github.com/propcgamer20-png), [#506](https://github.com/yakew7/Fair-Code/pull/506)) - a contributor could get a fully green `make check` locally and still fail the required `run-audits` CI check, or silently break the `faircode profile`/`faircode benchmark` CLI smoke tests. New `make audits` target runs all 7 audits' `unfair.py`/`fair.py` plus the CLI smoke tests CI actually runs, folded into `make check`.
- **`faircode/SPEC.md`'s own worked example ("`statecode` matches `state`") no longer matched the code** (closes #489, by [@propcgamer20-png](https://github.com/propcgamer20-png), [#505](https://github.com/yakew7/Fair-Code/pull/505)) - a prior fix added `EXACT_ONLY_KEYWORDS` (exact-match-only for `race`/`state`/`city`/`region`/`country`) to stop `state` from matching `statement`/`stateless`, which also broke this doc's own example without anyone updating it. Swapped to `zipcode`/`zip` and documented the exact-match carve-out inline so it can't silently drift again.
- **`make setup` never installed the `benchmark` extra** (closes #473, by [@propcgamer20-png](https://github.com/propcgamer20-png), [#480](https://github.com/yakew7/Fair-Code/pull/480)) - a contributor following `CONTRIBUTING.md`'s documented one-line setup step got `tests/test_benchmark.py`, `test_strategies.py`, `test_models.py`, `test_figures.py`, and `test_cli.py`'s benchmark subcommand tests all silently skipped via `pytest.importorskip`, with nothing telling them that happened. `make setup` now installs `benchmark`, matching what CI's own `benchmark-harness` job installs.
- **`compare --fail-on-drift` fired on a skipped/unmeasurable comparison** (closes #472) - it checked `bool(result["flags"])`, but `flags` also includes a kind-mismatch dimension's "drift comparison skipped" notice - genuinely informational, not evidence of drift (PSI 0.000, `drift_level` explicitly `"none"`). A schema change between two snapshots (a column reformatted, renamed, or dropped) falsely triggered "drift detected" in a CI gate. `compare()` now exposes a structural `drift_detected` field (every real drift signal, excluding kind-mismatch notices) and the CLI checks that instead.
- **MCP `proxy_hints`' `min_share`/`min_group_size` parameters were dead code** (closes #471) - the docstring claimed they were "the only two threshold knobs that feed dimension detection" for this tool, but `proxy_hints()` tests every detected dimension unconditionally, reading only each dimension's name/kind - fields neither parameter can change. Confirmed byte-identical output at the default vs. the most extreme possible values; removed the dead parameters entirely rather than inventing filtering semantics that don't exist anywhere else in the codebase.
- **`faircode/SPEC.md`'s `list_explainers` table row and docstring said it reads `assets/explainers-data.json`, contradicting the code** (closes #469) - the tool actually reads `faircode/_explainers/data.json`, the package-internal generated mirror built for the same reason as `faircode/_explainers/` itself (#388). Updated both to name the real path.
- **`profiler.html`'s `referenceStatus` had no `aria-live`, unlike its three sibling status elements** (closes #468) - a screen-reader user got no announcement that applying or clearing a reference baseline succeeded.
- **`profiler.html`'s dropzone nested a real `<button>` inside a `role="button"` container** (closes #467) - violated the HTML content model and WAI-ARIA's "interactive elements must not be nested" guidance. Moved the sample-dataset button out as a sibling, which also let two event-propagation workarounds that existed solely because of the nesting be removed as dead code.
- **A column-mapping override that invalidated the current cross-selection hid the entire results panel, trapping the user** (closes #466) - the mapping change handler called `reprofile()` and ignored its boolean return, unlike the reference-upload handler patched for exactly this class of problem in #419. It now checks the return value and clears the now-stale cross selection before retrying, instead of leaving the error (and the controls needed to fix it) hidden.
- **`parse_held_out_specs` silently dropped a duplicate held-out column name** (closes #463) - `held_out[column] = ...` overwrote silently when two `--proxy-hints-with` specs named the same column, unlike the sibling real-column collision check a few lines above. Now raises `ValueError`, mirroring that check's error style.
- **`compare()`'s displayed `psi` and `drift_level` could contradict each other at the rounding boundary** (closes #462) - classification ran on the unrounded float while `psi` displays the rounded value, so a true PSI of `0.09999...` (displayed as `0.1000`, at the documented moderate-drift threshold) classified as `"none"`. Both engines now classify on the same rounded value they display.
- **`compare()` was blind to a column collapsing to mostly-missing** (closes #461) - drift was computed purely from non-null group shares, so a column going from 0% to 95% missing produced zero measured PSI as long as the surviving rows kept the same split. Added `missing_pct_a`/`missing_pct_b`/`missing_pct_delta` per dimension and a new flag independent of the PSI-based `drift_level`.
- **`explainers-ui.js`'s client-side markdown renderer double-escaped `&` in link URLs** (closes #474) - same underlying bug as `build_explainers.py`'s server-side generator (#459), in the site's client-side renderer.
- **`build_explainers.py` double-escaped `&` in link URLs, breaking a real citation link on the live site** (closes #459) - `inline_markdown()` escaped the whole line first, then re-escaped an already-escaped URL substring extracted from it, turning `&` into `&amp;amp;`. Broke `automation-bias.md`'s real *State v. Loomis* citation link. Fixed by unescaping the extracted URL before resolving/re-escaping it.
- **`favicon.ico`'s 16x16 frame was a downscaled 32px render, not the dedicated 16px one** (closes #458) - 114 of 256 pixels differed from the dedicated `favicon-16x16.png`. Built from the 32px render with the 16px render passed via `append_images` instead.
- **`check_broken_links.py`'s link-existence check was silently case-insensitive on macOS/Windows** (closes #457) - `Path.exists()` accepted a case-mismatched relative link locally that git and the case-sensitive Linux CI runner this check gates would treat as broken. Now validated against `git ls-files`' own casing.
...
- **8 more non-explainer issues, each verified, fixed, and committed individually:**
  - **`faircode/SPEC.md`'s `get_benchmark_results` table cell said it wraps `paper/results-frozen/`** (closes #434) - the surrounding prose and `faircode/mcp_server.py` both say it reads the package-internal `faircode/_results_frozen/` mirror instead.
  - **`CITATION.cff`'s version/date were three releases behind `pyproject.toml`** (closes #435) - stuck at 2.0.0/2026-07-23 despite 2.1.0, 2.1.1, and 2.2.0 shipping since, including the entire MCP server and Phase 2 tool set.
  - **`ROADMAP.md`'s "Current release" was stale (v2.0.0) and its traction snapshot was labeled/dated August 2026** (closes #436) - refreshed both together so "last updated" doesn't immediately contradict the stats two lines below it.
  - **`llms.txt`/`llms-full.txt` still claimed the paper freeze is active and audits are paused** (closes #437) - the opposite of current, `CLAUDE.md`-documented policy. The stale sentence was hardcoded in `build_explainers.py`'s generator itself, so the existing "regenerate and diff" staleness check could never catch it.
  - **`.github/CODEOWNERS` had no entry for `CITATION.cff`, `SECURITY.md`, `CODE_OF_CONDUCT.md`, `LICENSE`, `llms.txt`, `llms-full.txt`, or `explainer.html`** (closes #438) - added owners matching their sibling top-level docs. `AGENTS.md`, also named in the original issue, turned out to be gitignored/untracked - not a real gap, left out; `tests/test_codeowners.py` caught this before it could land.
  - **`check_em_dash.py`'s allowlist for `faircode/significance.py` cited a paper freeze that's been lifted** (closes #442) - the file genuinely had two em dashes in its module docstring (prose, not frozen logic); fixed them and removed the stale allowlist entry.
  - **`strategies.py`'s comment above `EXPONENTIATED_GRADIENT_MAX_ITER` described a value (10) that hasn't been true since it was deliberately bumped to 50** (closes #445) - rewrote the comment to describe the actual current value instead of silently reverting benchmark-affecting behavior to match stale prose.
  - **`faircode/MANIFEST_SPEC.md` incorrectly said S3 needs `sensitive_features` at predict time** (closes #447) - only S4 does; `ExponentiatedGradient.predict` takes no such argument at all, verified directly against fairlearn's own signature.
- **`codeowners-access.yml` failed on any fork, spamming the fork owner's inbox** (by [@Shreyash0712](https://github.com/Shreyash0712), [#475](https://github.com/yakew7/Fair-Code/pull/475)) - the scheduled/push-triggered CODEOWNERS access check ran under a fork's own repo context too, trying (and failing) to verify write access for `@yakew7`/`@ahmdkaml`/etc. against the fork rather than the upstream repo. Now scoped to `github.repository == 'yakew7/Fair-Code'`, which still runs correctly for real PRs against this repo regardless of which fork they come from.
- **`render_terminal_png.py` leaked a raw traceback for a missing input file** (closes #460, by [@AchieverSana](https://github.com/AchieverSana), [#476](https://github.com/yakew7/Fair-Code/pull/476)) - the merged PR's `except FileNotFoundError` only covered a missing/moved path; passing a directory instead of a file (`IsADirectoryError`, a sibling `OSError` subclass) still leaked a traceback. Broadened directly (not part of the PR) to `except OSError`, using the OS's own `strerror` so the message stays accurate for either case instead of hardcoding "not found" for both; also added the test coverage the PR itself didn't include.
